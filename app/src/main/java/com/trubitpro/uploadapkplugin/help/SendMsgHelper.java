...
public final class SendMsgHelper {
    public static void sendMsgToLark(BaseVariant variant, Project project, String dataDTO, String gitLog, String gitBranch, FlutterGitBean flutterGitLog) {
        String defaultTitle = "测试包";
        String defaultText = "最新开发测试包已上传 ";
        String defaultClickText = "点我进行下载";
        String defaultLogTitle = "Git最近更新内容：\n ";
        SendLarkParams larkParams = SendLarkParams.getLarkParamsConfig(project);
        String webHookHostUrl = larkParams.webHookHostUrl;
        ...
        String title = larkParams.contentTitle;
        if (isEmpty(title)) {
            title = defaultTitle;
        }
        String titleStr = flavorStr;
        String text = larkParams.contentText;
        if (isEmpty(text)) {
            text = defaultText;
        }
        ...
        if ("interactive".equals(larkParams.msgtype)) {
            ...
            List<ElementsDTO> elementsDTOList2 = new ArrayList<>();
            ...
            textStr.append("![").append(isEmpty(larkParams.clickTxt) ? defaultClickText : larkParams.clickTxt)
...
//            textStr.append("[查看下载二维码]").append("(").append(QRCode).append(")");
            ...
            if (!isEmpty(gitLog) && larkParams.isSupportGitLog) {
                logStrBuilder.append("**").append("Current Branch：")
                              .append(gitBranch).append("**").append(defaultLogTitle).append("**")
                             .append(gitLog).append("\n");
            }
            if (larkParams.isBuildFlutter){
                logStrBuilder.append("**").append("Flutter Branch：").append(flutterGitLog.getFlutterGitBranch()).append("**").append(defaultLogTitle).append("**").append(flutterGitLog.getFlutterGitLog());
            }
            ...
            actionDownBtnText.setContent(isEmpty(larkParams.clickTxt) ? defaultClickText : larkParams.clickTxt);
            ...
            contentBeanA.setText(isEmpty(larkParams.clickTxt) ? defaultClickText : larkParams.clickTxt);
            ...
            StringBuilder logStrBuilder = new StringBuilder();
            if (!isEmpty(gitLog) && larkParams.isSupportGitLog) {
                logStrBuilder.append("**").append("Current Branch：").append(gitBranch).append("**").append(defaultLogTitle).append("**").append(gitLog).append("\n");
            }
            if (larkParams.isBuildFlutter) {
                logStrBuilder.append("**").append("Flutter Branch：").append(flutterGitLog.getFlutterGitBranch()).append("**").append(defaultLogTitle).append("**").append(flutterGitLog.getFlutterGitLog());
            }
